...
invertTensorEW: elementwise inversion of tensors accounting for zero elements
...
Authorship (class): loads and splits the dataset for the authorship attribution
    problem
powerGrid (class): loads the power grid data
...
LQR (class): creates trajectories of an LQR controller
...
class LQR(_data):
    """
    LQR: Creates a dataset consisting of trajectory of an LQR following the
        optimal controller
        x_{t+1} = A x_{t} + B u_{t}
        cost({(x_t, u_t)}_{t=1}^{T}) = x_T^T Q x_T
    ...
    Input:
        ...
        nValid (int): number of validation samples
        nTest (int): number of testing samples
        T (int): number of samples of the trajectories
        A (np.array): N x N matrix
        B (np.array): N x N matrix
        Q (np.array): N x N matrix
        R (np.array): N x N matrix
        x0 (optional, np.array): initial state, shape: (nTotal+nValid+nTest) x N
            If not specified, the initial state is selected from a random
    ...
    cost = .evaluate(x, u)
        Input:
            x (dtype.array): states of the system
                nSamples x tSamples (x 1) x nNodes
            u (dtype.array): controller applied to the system
                nSamples x tSamples (x 1) x nNodes
        Output:
            cost (float): compute the LQR cost as given by the loaded matrices
                Q and R
            cost({(x_t, u_t)}_{t=1}^{T}) = x_T^T Q x_T
                    + \sum_{t=0}^{T-1} (x_t^T Q x_t + u_t^T R u_t)
        Obs.: The cost is computed in either torch or numpy, directly. If it's
        computed in torch, it is capable of keeping track of the derivative
        graph. So if no gradients want to be accumulated, call .evaluate inside
        a torch.no_grad().

    x, u = .computeTrajectory(x0[, controller])
        Input:
            x0 (optional, dtype.array): initial state of the trajectory, if not
                specified, it considers the initial state given during the class
                initialization (i.e. (nTrain+nValid+nTest) x nNodes)
            controller (optional): if specified, it can be either a nn.Module
                with the specific architecture to be used as a controller; or
                a given tSamples x nNodes x nNodes matrix to use as a linear
                controller. If not specified, it resorts to the optimal linear
                centralized controller, i.e.
                u_{t} = -F_{t} x_{t}
                F_{t} = (R + B^{T} P_{t+1} B)^{-1} (B^{T} P_{t+1} A)
                P_{t-1} = A^{T} P_{k} A
                   - (A^{T} P_{t} B)(R + B^{T} P_{t} B)^{-1} (B^{T} P_{t} A) + Q
                P_{T} = Q
        Output:
            x (dtype.array): nTotal x tSamples x nNodes
                nTotal trajectories of the system (tSamples and nNodes are
                determined by the initialization of the class, nTotal is
                determined by the initial state)
            u (dtype.array): nTotal x (tSamples-1) x nNodes
                nTotal trajectories of the control signals
        Obs.: The trajectory is computed directly in either torch or numpy. If
        it's computed in torch, it is capable of keeping track of the derivative
        graph. So if no gradients want to be accumulated, call
        .computeTrajectory inside a torch.no_grad().

    radius = .computeRadiusOfAttraction([optionalArguments]): Estimates the
        region of attraction, i.e. for which value of input energy, the system
        becomes unstable. Instability is measured in terms of the derivative:
        if the derivative of the norm of the state does not go to zero (i.e. it
        ceases to be negative), then the system is not asymptotically stable.

        Input:
            nSamples (int, default: 1000): number of initial conditions, all
                with the same energy, to generate trajectories and see what
                happens after some time
            initRadius (float, default: 1.): initial energy of the input signals
            radiusProg ('sum' or 'prod', default: 'sum'): initRadius is
                increased arithmetically each time (if 'sum') or geometrically
                (if 'prod')
            radiusChange (float, default: 10.): number added to the previous
                radius (if radiusProg == 'sum') or multiplied by the previous
                radius (if radiusProg == 'prod')
            maxRadius (float, default: 1000.): when radius exceeds this number,
                and the system is still stable, it is considered that the system
                is stable for all signals
            zeroTolDeriv (float, default: 1-e3): numbers below this one are
                considered negative
            prob (float, default: 0.95): probability to guarantee that the
                system is stable (i.e. stableTrajectories/nSamples > prob to
                assert that a given energy radius is stable)
            controller (default: None): controller to use in order to compute
                the trajectory; it can be either a nn.Module, or a matrix (for
                a specific linear controller); see .computeTrajectory() for
                details.
            saveDir (string, default: None): If we want to keep track of the
                evolution of the trajectories that make the system unstable, we
                can save them in this directory. Note: without a saveDir
                specified in the input, the trajectories (the plots) will not be
                stored nor shown.
            modelName (string, default: None): If we want to save the plots,
                then we want to identify which model the plots are about, so we
                need to specify its name (this option is only used if a saveDir
                is provided).

        Obs.: The length of the trajectory, T, and the number of nodes N is
        specified at initialization time of the class, and cannot be changed
        (because all the controllers seek to minimize the quadratic cost at
        the specified end time)

    lowerBound = .computeLowerBound(S[, x0, alpha, doPrint, useMosek])
        Computes the lower bound on the optimal cost of the LQR problem using
        a linear decentralized controller
        Input:
            S (np.array, shape: N x N): GSO, the distributed controller will be
                assumed to respect this sparsity.
            x0 (np.array, shape: B x N, default: self.x0): Initial states to
                use for computing the lower bound
            alpha (float, default: 1.): penalty to ensure minimum rank of the
                surrogate W matrix
            doPrint (bool, default: True): show percentage progress of lower
                bound computation
            useMosek (bool, default: False): use MOSEK as a solver (requires
                license); default solver: 'SCS'.
        Output:
            lowerBound (np.array, shape: B): the corresponding lowerbound for
                the LQR problem as initialized by each one of the initial states
    ...
    .expandDims(): Adds the feature dimension to the graph signals (i.e. for
        graph signals of shape nSamples x tSamples x nNodes, turns them into
        shape nSamples x tSamples x 1 x nNodes, so that they can be handled by
        general graph signal processing techniques that take into account a
        feature dimension by default). It also adds the extra dimension to the
    ...
    def __init__(self, nTrain, nValid, nTest, T, A, B, Q, R, x0,
                 dataType = np.float64, device = 'cpu'):
        # Initialize parent
        super().__init__()
        # store attributes
        ...
        N = A.shape[0]
        assert len(B.shape) == 2 and B.shape[0] == B.shape[1] == N
        assert len(Q.shape) == 2 and Q.shape[0] == Q.shape[1] == N
        assert len(R.shape) == 2 and R.shape[0] == R.shape[1] == N
        self.N = N
        self.A = A
        self.B = B
        self.Q = Q
        ...
        #\\\ Optimal controller:
        # Create matrices required for the optimal controller
        # u_{t} = -F_{k} x_{t}
        # F_{t} = (R + B^{T} P_{t+1} B)^{-1} (B^{T} P_{t+1} A)
        # P_{t-1} = A^{T} P_{k} A - (A^{T} P_{t} B)(R + B^{T} P_{t} B)^{-1} (B^{T} P_{t} A) + Q
        # P_{T} = Q
        P = np.zeros((T, N, N))
        F = np.zeros((T-1, N, N)) # F[-1] is not defined, because u[-1] is not
        # defined, because x[-1] depends on u[-2] and not u[-1]
        P[-1] = Q
        for t in range(T-1):
            term1 = A.T @ P[T-t-1] @ A
            term2 = B.T @ P[T-t-1] @ A
            term3 = R + B.T @ P[T-t-1] @ B
            invTerm3 = np.linalg.inv(term3)
            P[T-t-2] =  term1 - \
                            term2.T @ invTerm3 @ term2 + \
                            Q
            F[T-t-2] =  invTerm3 @ term2

        self.P = P
        self.F = F

        #\\\ Compute trajectory
        x, u = self.computeTrajectory()

        # Split and save them
        self.samples['train']['signals'] = x[0:nTrain, :, :]
        self.samples['train']['targets'] = u[0:nTrain, :, :]
        self.samples['valid']['signals'] = x[nTrain:nTrain+nValid, :, :]
        self.samples['valid']['targets'] = u[nTrain:nTrain+nValid, :, :]
        self.samples['test']['signals'] = x[nTrain+nValid:nTotal, :, :]
        self.samples['test']['targets'] = u[nTrain+nValid:nTotal, :, :]
        # Change data to specified type and device
        self.astype(self.dataType)
        self.to(self.device)
    ...
    def expandDims(self):
        ...
        for key in self.samples.keys():
            ...
            if self.samples[key]['signals'] is not None:
                # And if it has only two dimensions
                #   (shape: nDataPoints x tSamples x nNodes)
                if len(self.samples[key]['signals'].shape) == 3:
                    # Then add a third dimension in between so that it ends
                    # up with shape
                    #   nDataPoints x tSamples x 1 x nNodes
                    # and it respects the 4-dimensional format that is taken
                    # by many of the processing functions
                    if 'torch' in repr(self.dataType):
                        self.samples[key]['signals'] = \
                        ...
                        self.samples[key]['signals'] = np.expand_dims(
                                                   self.samples[key]['signals'],
                                                   axis = 2)
            # And repeat for the control signals
            if self.samples[key]['targets'] is not None:
                if len(self.samples[key]['targets'].shape) == 3:
                    if 'torch' in repr(self.dataType):
                        self.samples[key]['targets'] = \
                                       self.samples[key]['targets'].unsqueeze(2)
                    else:
                        self.samples[key]['targets'] = np.expand_dims(
                                                   self.samples[key]['targets'],
                                                   axis = 2)

    def evaluate(self, x, u, reduce = True):
        # x and u are the trajectories. This just computes the cost

        # Check the length of the trajectories (whether we did expandDims or not)
        assert len(x.shape) == 3 or len(x.shape) == 4
        # If we expanded dims,
        if len(x.shape) == 4:
            # Check that the feature dimension is 1
            assert x.shape[2] == 1
            # And get rid of it
            x = x.squeeze(2)
        # Do the same with u
        ...
        tSamples = x.shape[1]
        nNodes = x.shape[2]
        assert u.shape[0] == nSamples
        assert u.shape[1] == tSamples-1
        assert u.shape[2] == nNodes

        # J = x_{T}^{T} Q x_{T} + \sum_{t=0}^{T-1} (x_{t}^{T} Q x_{t} + u_{t}^{T} R u_{t})

        # Now, depending on whether the data is torch or not, compute it in
        # torch or in numpy
        if 'torch' in repr(x.dtype):
            assert 'torch' in repr(u.dtype)
            device = x.device
            if u.device != device:
                u = u.to(device)
            # Move the matrices we need, Q and R, to be toch and in device
            if 'torch' not in repr(self.Q.dtype):
                Q = torch.tensor(self.Q).to(device)
            elif self.Q.device != device:
                Q = self.Q.to(device)
            else:
                Q = self.Q
            if 'torch' not in repr(self.R.dtype):
                ...
                R = self.R
            # Extract xT
            x, xT = torch.split(x, [tSamples-1, 1], dim = 1)
            # Technically, xT is shape nSamples x 1 x nNodes
            xTQ = torch.matmul(xT, Q) # nSamples x 1 x nNodes
            xTQxT = torch.matmul(xTQ, xT.permute(0,2,1)) # nSamples x 1 x 1
            xTQxT = xTQxT.squeeze(2).squeeze(1) # nSamples
            # Now let's go to the x and  u part
            # Both x and u are of size nSamples x (tSamples - 1) x nNodes
            # Let's add an extra dimension in the end to make them vectors
            x = x.unsqueeze(3) # nSamples x (tSamples - 1) x nNodes x 1
            u = u.unsqueeze(3) # nSamples x (tSamples - 1) x nNodes x 1
            xQx = torch.matmul(torch.matmul(x.permute(0,1,3,2), Q), x)
                # nSamples x (tSamples-1) x 1 x 1
            xQx = xQx.squeeze(3).squeeze(2) # nSamples x (tSamples-1)
            uRu = torch.matmul(torch.matmul(u.permute(0,1,3,2), R), u)
                # nSamples x (tSamples-1) x 1 x 1
            uRu = uRu.squeeze(3).squeeze(2) # nSamples x (tSamples-1)
            xQxSuRu = xQx + uRu
            xQxSuRu = torch.sum(xQxSuRu, dim = 1) # nSamples
            cost = xTQxT + xQxSuRu
            if reduce: # If reduce is true, avreage over the samples
                cost = torch.mean(cost)
            # Now do the computation for numpy
        else:
            # If needed make the matrices be numpy
            if 'torch' in repr(self.Q.dtype):
                Q = self.Q.cpu().numpy()
            else:
                Q = self.Q
            if 'torch' in repr(self.R.dtype):
                R = self.R.cpu().numpy()
            else:
                R = self.R
            # Extract xT
            xT = np.expand_dims(x[:,-1,:], 2) # nSamples x nNodes x 1
            xTQxT = xT.transpose(0,2,1) @ Q @ xT # nSamples x 1 x 1
            xTQxT = xTQxT.squeeze(2).squeeze(1) # nSamples
            # Use the other values of x
            x = x[:,0:tSamples-1,:] # nSamples x (tSamples - 1) x nNodes
            x = np.expand_dims(x, 3) # nSamples x (tSamples - 1) x nNodes x 1
            u = np.expand_dims(u, 3) # nSamples x (tSamples - 1) x nNodes x 1
            xQx = x.transpose(0,1,3,2) @ Q @ x
                # nSamples x (tSamples - 1) x 1 x 1
            uRu = u.transpose(0,1,3,2) @ R @ u
                # nSamples x (tSamples - 1) x 1 x 1
            xQxSuRu = xQx + uRu
            xQxSuRu = np.sum(xQxSuRu, axis = 1) # nSamples
            cost = xTQxT + xQxSuRu
            if reduce: # If reduce is true, average over the samples
                cost = np.mean(cost)

        return cost

    def computeTrajectory(self, x0 = None, controller = None, **kwargs):

        # It can be computed based on an architecture or on a given matrix;
        # if none is given, then it will use the stored matrix (the optimal
        # centralized controller)
        useArchit = False
        useMatrix = False

        # I can't change the T because the computed optimal matrices depend on
        # that value of T
        # I could only change the initial value
        N = self.A.shape[0]
        if x0 is None:
            x0 = self.x0
        ...
        # Check if the data is stored in torch or not
        if 'torch' in repr(x0.dtype):
            useTorch = True
        else:
            useTorch = False

        # If we're going to compute the learned controller
        ...
        elif 'torch.nn.modules.module.Module' in repr(type(controller).__bases__):
            # Save the archit (for the forward)
            archit = controller
            # Get the device
            architDevice = list(archit.parameters())[0].device
            # Flag it
            useArchit = True
            # There won't be any matrix used
            F = None
        else: # It has to be a matrix
            F = controller
            assert len(F.shape) == 3 and F.shape[1] == F.shape[2] == N
            useMatrix = True
        ...
        # Generate the samples
        if useTorch or useArchit:
            # Conver the variables to use
            if 'torch' not in repr(x0.dtype):
                x0 = torch.tensor(x0).to(device)
            else:
                x0 = x0.to(device)
            if 'torch' not in repr(self.A.dtype):
                A = torch.tensor(self.A).to(device)
            else:
                A = self.A.to(device)
            if 'torch' not in repr(self.B.dtype):
                B = torch.tensor(self.B).to(device)
            else:
                B = self.B.to(device)
            if F is not None:
                if 'torch' not in repr(F.dtype):
                    F = torch.tensor(F).to(device)
                else:
                    F = F.to(device)
            # Create the place
            x = x0.unsqueeze(1) # nTotal x 1 x N
            u = torch.empty(nTotal, 0, N).to(device)
            xt = x0 # The first time instant

            for t in range(1,self.T):
                if useArchit:
                    # Convert to B x 1 x 1 x N to get the output of the same
                    # size. It's for each time instant in this case
                    ut = archit(xt.reshape((nTotal,1,1,N))).reshape((nTotal,N))
                elif useMatrix:
                    ut = torch.matmul(xt, F[t-1].t())
                xt = torch.matmul(xt, A.t()) + torch.matmul(ut, B.t())
                x = torch.cat((x,xt.unsqueeze(1)), dim = 1)
                u = torch.cat((u,ut.unsqueeze(1)), dim = 1)

            # If the original data was not on torch -the initial states-, then
            # don't return a torch
            if not useTorch:
                x = x.detach().cpu().numpy()
                u = u.detach().cpu().numpy()
        else:
            # Convert the variables to use
            if 'torch' in repr(x0.dtype):
                x0 = x0.cpu().numpy()
            if 'torch' in repr(self.A):
                A = A.cpu().numpy()
            else:
                A = np.array(self.A)
            if 'torch' in repr(self.B):
                B = B.cpu().numpy()
            else:
                B = np.array(self.B)
            if 'torch' in repr(F):
                ...
                F = np.array(F)
            # Create the space
            x = np.zeros((nTotal, self.T, N))
            u = np.zeros((nTotal, self.T-1, N))
            # Initial state
            x[:, 0, :] = x0

            # Trajectory
            for t in range(1,self.T):
                # Since we are in the no-architecture option, this cannot be
                # the architecture, but just the linear model
                u[:, t-1, :] = x[:, t-1, :].dot(-F[t-1].T)
                x[:, t, :] = x[:, t-1, :].dot(A.T) + u[:, t-1, :].dot(B.T)

        return x, u

    def computeLowerBound(self, S, x0 = None, alpha = 1.,
                          doPrint = True, useMosek = False):

        if 'torch' in repr(S.dtype):
            S = S.cpu().numpy()
        # Convert S into the "one-zero matrix" L
        L = (S>0).astype(S.dtype)

        # Check what initial points to use, if some externally provided ones,
        # or to use the stored ones
        if x0 is None:
            x0 = self.x0
        assert len(x0.shape) == 2
        nTotal = x0.shape[0]
        assert x0.shape[1] == self.N

        if 'torch' in repr(x0.dtype):
            x0 = x0.cpu().numpy()

        N = self.N
        if 'torch' in repr(self.A.dtype):
            A = self.A.cpu().numpy()
        else:
            A = self.A
        if 'torch' in repr(self.B.dtype):
            B = self.B.cpu().numpy()
        else:
            B = self.B
        if 'torch' in repr(self.Q.dtype):
        ...
        if useMosek and 'MOSEK' in cp.installed_solvers():
            Solver = 'MOSEK'
        else:
            Solver = 'SCS'

        mu = 0.99 * np.min(np.linalg.eigvals(Q))
        QT = np.real(scipy.linalg.sqrtm(Q - mu*np.eye(N)))
        RT = np.real(scipy.linalg.sqrtm(R))
        Spar = L @ L.T

        lowerBound = [] # Store the values of the lower bound for each initial
            # state

        if doPrint:
            print("  0%", flush = True, end = '')
        # For each initial state x0, we need to compute the lower bound
        for b in range(nTotal):

            W = cp.Variable((2*N+T+1, 2*N+T+1), PSD = True) # Surrogate variable
                # for SDP problem

            # Variables obtained from W
            constraints = [W[0:N, 0:N] == np.eye(N)]
            X = W[0:N, N:(N+T+1)] # X = [x_0, ..., x_t, ..., x_T]
            U = W[(N+T+1):(2*N+T+1), N:(N+T+1)] # U = [u_0, ..., u_t, ..., u_T]
            K = W[(N+T+1):(2*N+T+1), 0:N] # Linear controller solution
            W22 = W[N:(N+T+1), N:(N+T+1)]
            W33 = W[(N+T+1):(2*N+T+1), (N+T+1):(2*N+T+1)]

            # Systems constraints
            constraints += [X[:,0] == x0[b,:]]
            for t in range(T):
            ...
            for i in range(N):
                for j in range(N):
                    if Spar[i,j] == 0:
                        constraints += [W33[i,j] == 0]
                    if L[i,j] == 0:
                        constraints += [K[i,j] == 0]

            prob = cp.Problem(
                cp.Minimize(
                    cp.sum(cp.vec(QT @ X) ** 2) + \
                    cp.sum(cp.vec(RT @ U) ** 2) + \
                    alpha * cp.trace(W33) +\
                    mu * cp.trace(W22)),
                constraints)
            ...
            thisLowerBound = np.trace(X.T @ Q @ X) + np.trace(U.T @ R @ U)

            lowerBound += [thisLowerBound]

            del W, X, U, prob, constraints, thisLowerBound

            if doPrint:
                print("\b\b\b\b%3d%%" % (b/nTotal * 100), end = '',flush = True)

        if doPrint:
            print("\b\b\b\b", end = '', flush = True)

        return np.array(lowerBound)


    def astype(self, dataType):

        # Change A, B, Q, R and x0. Also, P and F
        self.A = changeDataType(self.A, dataType)
        self.B = changeDataType(self.B, dataType)
        self.Q = changeDataType(self.Q, dataType)
        self.R = changeDataType(self.R, dataType)
        self.P = changeDataType(self.P, dataType)
        self.F = changeDataType(self.F, dataType)
        self.x0 = changeDataType(self.x0, dataType)
...
class PowerGrid(_data):
    """
    PowerGrid: Handles a 39-bus power grid with active and reactive power
        measurements and voltage magnitudes; the state of the grid is given
        by voltage magnitudes and voltage angles.

    Initialization:

    Input:
        ratioTrain (float): ratio of the total measurements to be part of the
            training set
        ratioValid (float): ratio of the training measurements to be part of the
            validation set
        dataPath (string): path to where the power grid state data is located
        nSamples (int, default: None): If less samples want to be used, specify
            this number
        dataType (dtype, default: np.float64): type of loaded data
        device (device, default: 'cpu'): where to store the data when the
            dataType is a torch.tensor (e.g., 'cpu', 'cuda:0', etc.)

    Methods:

    .loadData(dataPath): load the data found in dataPath and store it in
        attributes .measurement, .state, .admittanceMatrix, .flowMatrix
        and .nBuses

    .assignData(randPerm = None): assigns the data to the corresponding
        training, validation and testing sets in the built-in attributes of
        .samples. If randPerm = None, a new random permutation is created and
        stored in the attribute .randPerm. If not, the array randPerm is used
        to sort the sets.
    ...
    admittanceMatrix = .getAdmittanceMatrix(): returns the
            2 x .nBuses x .nBuses
        admittance matrix, where admittanceMatrix[0] corresponds to the real
        part of the admittance and admittanceMatrix[1] to the imaginary part

    flowMatrix = .getSamples(samplesType[, optionalArguments])
        Input:
            samplesType (string): 'all', 'train', 'valid' or 'test' to
                determine from which dataset to get the samples from
            optionalArguments:
                0 optional arguments: get all the samples from the specified set
                1 optional argument (int): number of samples to get (at random)
                1 optional argument (list): specific indices of samples to get
        Output:
            flowMatrix (dtype.array): numberSamples x 4 x nBuses x nBuses
            labels (dtype.array): numberSamples
            >> Obs.: The 0th dimension matches the corresponding signal to its
                respective label

    nBuses = .getNumberOfBuses(): returns an int that indicates the total
        number of buses in th power grid
    ...
    def __init__(self, ratioTrain, ratioValid, dataPath, nSamples = None,
                 dataType = np.float64, device = 'cpu'):
        # Initialize parent
        super().__init__()
        # Store
        ...
        self.state = None
        self.admittanceMatrix = None
        self.flowMatrix = {}
        self.flowMatrix['all'] = None
        self.flowMatrix['train'] = None
        self.flowMatrix['valid'] = None
        self.flowMatrix['test'] = None
        self.randPerm = None
        self.nBuses = None
        # Load data
        self.loadData(dataPath)
    ...
    def loadData(self, dataPath):
        ...
        # Load data (from a .mat file)
        rawData = hdf5storage.loadmat(dataPath)
        # rawData is a dictionary with several keys:
        #   real_Ybus: real part of the admittance matrix
        #       (dimension: nxn)
        #   imag_Ybus: imaginary part of the admittance matrix
        #       (dimension: nxn)
        #   x_data: true underlying states
        #       (dimension: 2n-1 x n_data)
        #   p_data: real power injection measurements at nodes
        #       (dimension: n x n_data)
        #   q_data: reactive power injection measurements at nodes
        #       (dimension: n x n_data)
        #   pf_data: real power flow measurements at from-node side
        #       (dimension: n x n x n_data)
        #   qf_data: reactive power flow measurements at from-node side
        #       (dimension: n x n x n_data)
        #   pt_data: real power flow measurements at to-node side
        #       (dimension: n x n x n_data)
        #   qt_data: reactive power flow measurements at to-node side
        #       (dimension: n x n x n_data)
        # The issue is that hdf5storage, while necessary to load old
        # Matlab(R) files, gives the data in a weird format, that we need
        # to adapt and convert.
        # The data will be structured as follows.
        # The signals are the measurements, which are two:
        #   real power (p_data) and reactive power (q_data)
        # which will be concatenated in a signal with two features
        # The targets are also two for each node, both contained in x_data
        # The admittance matrix will have two features, obtained from
        #   real_Ybus and imag_Ybus
        # The power flow matrix will have two features:
        #   pf_data and pt_data, and qf_data and qt_data
        #!!!! We will need to discuss how to combine the admittance with the edge
        # measurements.
        # Let's start with the signals
        p_data = rawData['p_data'].T # B x N
        q_data = rawData['q_data'].T # B x N
        measurement = np.concatenate((np.expand_dims(p_data, 1),
                                      np.expand_dims(q_data, 1)), axis = 1) #Bx2xN
        B = measurement.shape[0] # total data points
        N = measurement.shape[2] # total number of nodes
        # Add the targets
        x_data = rawData['x_data']
        # The first N values correspond to the voltage measurements, the
        # following (N-1) values correspond to the angle measurements, with
        # the first node set to zero
        v_data = x_data[0:N,:].T # B x N
        phi_data = x_data[N:,:].T # B x (N-1)
        # Add the first zero
        phi_data = np.concatenate((np.zeros((B,1)), phi_data), axis = 1) # BxN
        # And concatenate them to make them targets
        state = np.concatenate((np.expand_dims(v_data, 1),
                                np.expand_dims(phi_data, 1)), axis = 1) #Bx2xN
        # And also add the voltage measurements to the signal vector as well
        measurement = np.concatenate((measurement,
                                      np.expand_dims(v_data, 1)),
                                     axis = 1) # B x 3 x N

        # Now let's move to the graph topology matrices
        real_Ybus = rawData['real_Ybus'] # N x N
        imag_Ybus = rawData['imag_Ybus'] # N x N
        admittanceMatrix = np.concatenate((np.expand_dims(real_Ybus, 0),
                                           np.expand_dims(imag_Ybus, 0)),
                                          axis = 0) # 2 x N x N

        # Flow matrix
        pf_data = np.transpose(rawData['pf_data'], (2, 0, 1)) # B x N x N
        qf_data = np.transpose(rawData['qf_data'], (2, 0, 1)) # B x N x N
        pt_data = np.transpose(rawData['pt_data'], (2, 0, 1)) # B x N x N
        qt_data = np.transpose(rawData['qt_data'], (2, 0, 1)) # B x N x N
        # Let's put all of them separately for now
        flowMatrix = np.concatenate((np.expand_dims(pf_data, 1),
                                     np.expand_dims(qf_data, 1),
                                     np.expand_dims(pt_data, 1),
                                     np.expand_dims(qt_data, 1)),
                                    axis = 1) # B x 4 x N x N

        # Now, save everything were it goes
        self.measurement = measurement
        self.state = state
        self.admittanceMatrix = admittanceMatrix
        self.flowMatrix['all'] = flowMatrix
        self.nBuses = N

    def assignData(self, randPerm = None):

        # If we didn't specify the number of samples, or we specified a
        # number larger than the samples available, just select all available
        if self.nSamples is None or self.nSamples >= self.measurement.shape[0]:
            # Get the number of data points
            nDataPoints = self.measurement.shape[0]
            # And update the value
            self.nSamples = nDataPoints
        # If we did specify them and they're less than those available
        else:
            nDataPoints = self.nSamples

        # Compute the number of training, validation and testing points
        self.nTest = int(round((1-self.ratioTrain) * nDataPoints))
        self.nValid = int(round(self.ratioTrain * self.ratioValid * nDataPoints))
        self.nTrain = nDataPoints - self.nTest - self.nValid

        if randPerm is None:
            # Get a random permutation of these works, and split them
            # accordingly
            # Recall that we can choose any samples from all availables, even
            # if we later select only a few
            randPerm = np.random.permutation(self.measurement.shape[0])
        else:
            assert np.max(randPerm) < self.measurement.shape[0]
        # Save the indices corresponding to each split
        randPermTrain = randPerm[0 : self.nTrain]
        randPermValid = randPerm[self.nTrain : self.nTrain + self.nValid]
        randPermTest = randPerm[self.nTrain + self.nValid : nDataPoints]
        measurementTrain = self.measurement[randPermTrain, :]
        measurementValid = self.measurement[randPermValid, :]
        measurementTest = self.measurement[randPermTest, :]
        stateTrain = self.state[randPermTrain, :]
        stateValid = self.state[randPermValid, :]
        stateTest = self.state[randPermTest, :]
        flowMatrixTrain = self.flowMatrix['all'][randPermTrain, :]
        flowMatrixValid = self.flowMatrix['all'][randPermValid, :]
        flowMatrixTest = self.flowMatrix['all'][randPermTest, :]
        # And assign them to the required attribute samples
        self.samples['train']['signals'] = measurementTrain
        self.samples['train']['targets'] = stateTrain
        self.samples['valid']['signals'] = measurementValid
        self.samples['valid']['targets'] = stateValid
        self.samples['test']['signals'] = measurementTest
        self.samples['test']['targets'] = stateTest
        # We also need to partition the flow matrix
        self.flowMatrix['train'] = flowMatrixTrain
        self.flowMatrix['valid'] = flowMatrixValid
        self.flowMatrix['test'] = flowMatrixTest
        # Save the random partition
        self.randPerm = randPerm

    def getAdmittanceMatrix(self):

        return self.admittanceMatrix

    def getFlowMatrix(self, samplesType, *args):
    ...
    def getNumberOfBuses(self):

        return self.nBuses

    def evaluate(self, yHat, y):

        # Let's take the GS norm difference between signals, i.e., the L2,1
        # matrix norm of the N x F matrix
        ...
        if 'torch' in repr(self.dataType):
            # Compute the ||x^f||_2^2 the squared 2-norm of each feature
            GSnorm = torch.sum((y - yHat) ** 2, dim = 2) # B x 2
            # Apply the sqrt to obtain the 2-norm of each feature ||x_f||_2
            GSnorm = torch.sqrt(GSnorm) # B x 2
            # Add over all features: \sum_f ||x_f||_2
            GSnorm = torch.sum(GSnorm, dim = 1) # B
            # Average over the batches
            meanGSnorm = torch.mean(GSnorm)
        else:
            GSnorm = np.sum((y - yHat) ** 2, axis = 2) # B x 2
            GSnorm = np.sqrt(GSnorm) # B x 2
            GSnorm = np.sum(GSnorm, axis = 1) # B
            meanGSnorm = np.mean(GSnorm)

        return meanGSnorm


    def astype(self, dataType):
        self.measurement = changeDataType(self.measurement, dataType)
        self.state = changeDataType(self.state, dataType)
        self.admittanceMatrix = changeDataType(self.admittanceMatrix, dataType)
        for key in self.flowMatrix.keys():
            self.flowMatrix[key] = changeDataType(self.flowMatrix[key], dataType)
        # And now, initialize to change the samples as well (and also save the
        # data type)
        super().astype(dataType)
    ...
    def to(self, device):
        ...
        if 'torch' in repr(self.dataType):
            ...
            self.admittanceMatrix.to(device)
            self.flowMatrix.to(device)
            # And call the inherit method to initialize samples (and save to
            # device)
            super().to(device)
    ...
    signals, targets = .getSamples(samplesType[, optionalArguments])
        Input:
            ...
            optionalArguments:
                ...
                1 optional argument (list): specific indices of samples to get
        Output:
            signals (dtype.array): numberSamples x tSamples x 6 x numberNodes
            targets (dtype.array): numberSamples x tSamples x 2 x numberNodes
